// node_modules/@fuels-ts/program/dist/index.js
...
var RevertErrorCodes = class {
    ...
    constructor(receipts, logs) {
        ...
        this.logs = logs
        // console.log("RevertErrorCodes logs:", logs);
    }
...
var InstructionSet = class {
    ...
    toHex() {
        return (0, import_utils2.hexlify)(this.toBytes())
    }
    toString() {
        return `Program:
...
var calculateScriptDataBaseOffset = (maxInputs) =>
    import_abi_coder.SCRIPT_FIXED_SIZE + (0, import_abi_coder.calculateVmTxMemory)({ maxInputs })
var POINTER_DATA_OFFSET =
    import_abi_coder.WORD_SIZE +
    import_abi_coder.ASSET_ID_LEN +
    import_abi_coder.CONTRACT_ID_LEN +
    import_abi_coder.WORD_SIZE +
    import_abi_coder.WORD_SIZE
...
function callResultToScriptResult(callResult) {
    ...
    receipts.forEach((receipt) => {
        if (receipt.type === import_transactions3.ReceiptType.ScriptResult) {
            ...
            receipt.type === import_transactions3.ReceiptType.Return ||
            receipt.type === import_transactions3.ReceiptType.ReturnData ||
            receipt.type === import_transactions3.ReceiptType.Revert
        ) {
...
function callResultToInvocationResult(callResult, call2, logs) {
    return decodeCallResult(
        ...
        (scriptResult) => {
            ...
            if (
                scriptResult.returnReceipt.type !== import_transactions3.ReceiptType.Return &&
                scriptResult.returnReceipt.type !== import_transactions3.ReceiptType.ReturnData
            ) {
...
var ScriptRequest = class {
    ...
    constructor(bytes, scriptDataEncoder, scriptResultDecoder2) {
        this.bytes = (0, import_utils3.arrayify)(bytes)
    ...
    encodeScriptData(data) {
        ...
        this.bytes = (0, import_utils3.arrayify)(callScript.script)
...
var getSingleCallInstructions = ({ callDataOffset, gasForwardedOffset, amountOffset, assetIdOffset }, outputInfo) => {
    ...
    if (gasForwardedOffset) {
        inst.push(asm.movi(19, gasForwardedOffset), asm.lw(19, 19, 0), asm.call(16, 17, 18, 19))
    } else {
...
var isReturnType = (type) =>
    type === import_transactions4.ReceiptType.Return || type === import_transactions4.ReceiptType.ReturnData
var getMainCallReceipt = (receipts, contractId) =>
    receipts.find(
        ({ type, from, to }) =>
            type === import_transactions4.ReceiptType.Call && from === SCRIPT_WRAPPER_CONTRACT_ID && to === contractId,
    )
...
var SCRIPTResultDecoder = (contractId, isOutputDataHeap) => (result) => {
    ...
    const mainCallResult = getMainCallReceipt(result.receipts, contractId.toB256())
    const mainCallInstructionStart = (0, import_math.bn)(mainCallResult?.is)
    const receipts = result.receipts
    return receipts
        .filter(({ type }) => isReturnType(type))
        .flatMap((receipt, index, filtered) => {
            ...
            if (receipt.type === import_transactions4.ReceiptType.ReturnData) {
                const encodedScriptReturn = (0, import_utils4.arrayify)(receipt.data)
                if (isOutputDataHeap && isReturnType(filtered[index + 1]?.type)) {
                    const nextReturnData = filtered[index + 1]
                    return (0, import_utils4.concat)([encodedScriptReturn, (0, import_utils4.arrayify)(nextReturnData.data)])
                }
...
var decodeContractCallScriptResult = (callResult, contractId, isOutputDataHeap, logs = []) =>
    decodeCallResult(callResult, SCRIPTResultDecoder(contractId, isOutputDataHeap), logs)

var getCallInstructionsLength = (contractCalls) =>
...
var getFunctionOutputInfos = (functionScopes) =>
...
var getContractCallScript = (functionScopes, maxInputs) =>
    new ScriptRequest(
        // Script to call the contract, start with stub size matching length of calls
        getInstructions(new Array(functionScopes.length).fill(DEFAULT_OPCODE_PARAMS), getFunctionOutputInfos(functionScopes)),
        (contractCalls) => {
            ...
            for (let i = 0; i < TOTAL_CALLS; i += 1) {
                ...
                paramOffsets.push({
                    ...
                    gasForwardedOffset: call2.gas
                        ? segmentOffset + import_abi_coder2.WORD_SIZE + import_abi_coder2.ASSET_ID_LEN
                        : 0,
                    callDataOffset:
                        segmentOffset + import_abi_coder2.WORD_SIZE + import_abi_coder2.ASSET_ID_LEN + gasForwardedSize,
                })
                scriptData.push(new import_abi_coder2.BigNumberCoder("u64").encode(call2.amount || 0))
                scriptData.push(
                    new import_abi_coder2.B256Coder().encode(call2.assetId?.toString() || import_configs3.BaseAssetId),
                )
                scriptData.push(call2.contractId.toBytes())
                scriptData.push(new import_abi_coder2.BigNumberCoder("u64").encode(call2.fnSelector))
                if (call2.gas) {
                ...
                if (call2.isInputDataPointer) {
                    ...
                    scriptData.push(new import_abi_coder2.BigNumberCoder("u64").encode(pointerInputOffset))
                }
                const args = (0, import_utils4.arrayify)(call2.data)
                scriptData.push(args)
                segmentOffset = dataOffset + (0, import_utils4.concat)(scriptData).byteLength
            }
            const script = getInstructions(paramOffsets, outputInfos)
            const finalScriptData = (0, import_utils4.concat)(scriptData)
...
var InvocationResult = class {
    ...
    getDecodedLogs(receipts) {
        ...
        return (0, import_account.getDecodedLogs)(receipts, program.interface)
    }
}
var FunctionInvocationResult = class extends InvocationResult {
    ...
    static async build(funcScope, transactionResponse, isMultiCall, program) {
        ...
        const fnResult = new FunctionInvocationResult(funcScope, transactionResponse, txResult, program, isMultiCall)
        return fnResult
    }
}
...
var BaseInvocationScope = class {
    ...
    constructor(program, isMultiCall) {
        this.program = program
        this.isMultiCall = isMultiCall
        this.transactionRequest = new import_account2.ScriptTransactionRequest()
    }
    /**
    ...
    updateRequiredCoins() {
        ...
        this.requiredCoins = Array.from(assets.reduce(reduceForwardCoins, /* @__PURE__ */ new Map()).values())
    }
    /**
    ...
    async getTransactionCost(options) {
        ...
        request.gasPrice = (0, import_math3.bn)(
            (0, import_math3.toNumber)(request.gasPrice) || (0, import_math3.toNumber)(options?.gasPrice || 0),
        )
    ...
    async fundWithRequiredCoins() {
        ...
        const { maxFee, gasUsed, minGasPrice, estimatedInputs, outputVariables, missingContractIds, requiredQuantities } =
        ...
        missingContractIds.forEach((contractId) => {
            this.transactionRequest.addContractInputAndOutput(import_address.Address.fromString(contractId))
        })
    ...
    async call() {
        ...
        const response = await this.program.account.sendTransaction(await this.getTransactionRequest(), {
            awaitExecution: true,
            estimateTxDependencies: false,
        })
        return FunctionInvocationResult.build(this.functionInvocationScopes, response, this.isMultiCall, this.program)
    }
    /**
    ...
    async simulate() {
        ...
        await this.fundWithRequiredCoins()
        const result = await this.program.account.simulateTransaction(await this.getTransactionRequest(), {
            estimateTxDependencies: false,
        })
    ...
    async dryRun() {
        ...
        return InvocationCallResult.build(this.functionInvocationScopes, response, this.isMultiCall)
    }
    getProvider() {
    ...
    setDefaultTxParams(transactionRequest, minGasPrice, gasUsed) {
        ...
        if (!gasLimitSpecified) {
            ...
            throw new import_errors5.FuelError(
                import_errors5.ErrorCode.GAS_LIMIT_TOO_LOW,
        ...
        if (!gasPriceSpecified) {
            ...
            throw new import_errors5.FuelError(
                import_errors5.ErrorCode.GAS_PRICE_TOO_LOW,
...
var FunctionInvocationScope = class extends BaseInvocationScope {
    ...
    callParams(callParams) {
        ...
        if (callParams?.forward) {
            ...
            this.forward = (0, import_account3.coinQuantityfy)(callParams.forward)
        }
