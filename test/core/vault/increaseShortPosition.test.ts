...
import { BN, DateTime, FUEL_NETWORK_URL, Provider, Wallet, WalletUnlocked } from "fuels"
import {
    FungibleAbi,
    GlpAbi,
    GlpManagerAbi,
...
describe("Vault.increaseShortPosition", function () {
    ...
    beforeEach(async () => {
        ...
        */
        ...
        usdg = await deploy("Usdg", deployer)
        router = await deploy("Router", deployer)
        timeDistributor = await deploy("TimeDistributor", deployer)
        yieldTracker = await deploy("YieldTracker", deployer)
        glp = await deploy("Glp", deployer)
        glpManager = await deploy("GlpManager", deployer)

        await usdg.functions.initialize(toContract(vault)).call()
        await router.functions.initialize(toContract(vault), toContract(usdg), toAddress(deployer)).call()
        await vaultStorage.functions
            .initialize(
                addrToAccount(deployer),
                toContract(router),
                toAsset(usdg), // USDG native asset
                toContract(usdg), // USDG contract
            ...
            .call()
        await vault.functions.initialize(addrToAccount(deployer), toContract(vaultStorage)).call()
        await vaultStorage.functions.write_authorize(contrToAccount(vault), true).call()

        await yieldTracker.functions.initialize(toContract(usdg)).call()
        ...
        await usdg.functions.set_yield_trackers([contrToAccount(yieldTracker)]).call()
        ...
        await glp.functions.initialize().call()
        await glpManager.functions
            .initialize(
                toContract(vault),
    ...
    it("increasePosition short validations", async () => {
        ...
        await expect(
            vault.connect(user1).functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), 0, false).call(),
        ).to.be.revertedWith("VaultInvalidMsgCaller")
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(1000), false)
                .call(),
        ).to.be.revertedWith("VaultShortCollateralAssetNotWhitelisted")
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(BNB), toAsset(BNB), toUsd(1000), false)
                .call(),
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(DAI), toUsd(1000), false)
                .call(),
        ...
        await expect(
            vault.connect(user0).functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), 0, false).call(),
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(100), false)
                .call(),
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(8), false)
                .call(),
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(600), false)
                .call(),
    ...
    it("increasePosition short", async () => {
        ...
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("0")
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("0")
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(99), false)
                .call(),
        ).to.be.revertedWith("VaultSizeMustBeMoreThanCollateral")
        ...
        await expect(
            vault
                .connect(user0)
                .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(501), false)
                .call(),
        ).to.be.revertedWith("VaultReserveExceedsPool")

        expect(await getValStr(vaultStorage.functions.get_fee_reserves(toAsset(DAI)))).eq("0")
        expect(await getValStr(vaultStorage.functions.get_usdg_amount(toAsset(DAI)))).eq("0")
        expect(await getValStr(vaultStorage.functions.get_pool_amounts(toAsset(DAI)))).eq("0")

        expect(await getValStr(vault.functions.get_redemption_collateral_usd(toAsset(DAI)))).eq("0")
        await vault.functions.buy_usdg(toAsset(DAI), addrToAccount(user1)).call()
        ...
        expect(await getValStr(vaultStorage.functions.get_usdg_amount(toAsset(DAI)))).eq("49980000000") // 499.8
        expect(await getValStr(vaultStorage.functions.get_pool_amounts(toAsset(DAI)))).eq("49980000000") // 499.8

        globalDelta = formatObj(await getValue(vault.functions.get_global_short_delta(toAsset(BTC))))
        expect(await globalDelta[0]).eq(false)
        expect(await globalDelta[1]).eq("0")
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("49980000000")
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("49980000000")
        ...
        expect(await getValStr(vaultStorage.functions.get_reserved_amounts(toAsset(BTC)))).eq("0")
        expect(await getValStr(vaultStorage.functions.get_guaranteed_usd(toAsset(BTC)))).eq("0")

        let position = formatObj(
            await getValue(vault.functions.get_position(toAddress(user0), toAsset(DAI), toAsset(BTC), false)),
        )
        ...
        await vault
            .connect(user0)
            .functions.increase_position(toAddress(user0), toAsset(DAI), toAsset(BTC), toUsd(90), false)
            .call()

        expect(await getValStr(vaultStorage.functions.get_pool_amounts(toAsset(DAI)))).eq("49980000000")
        expect(await getValStr(vaultStorage.functions.get_reserved_amounts(toAsset(DAI)))).eq(expandDecimals(90))
        expect(await getValStr(vaultStorage.functions.get_guaranteed_usd(toAsset(DAI)))).eq("0")
        expect(await getValStr(vault.functions.get_redemption_collateral_usd(toAsset(DAI)))).eq(
            "499800000000000000000000000000000",
        )
        ...
        position = formatObj(await getValue(vault.functions.get_position(toAddress(user0), toAsset(DAI), toAsset(BTC), false)))
        ...
        expect(await getValStr(vaultStorage.functions.get_usdg_amount(toAsset(DAI)))).eq("49980000000") // 0.29
        expect(await getValStr(vaultStorage.functions.get_pool_amounts(toAsset(DAI)))).eq("49980000000") // 499.8

        expect(await getValStr(vaultStorage.functions.get_global_short_sizes(toAsset(BTC)))).eq(toUsd(90))
        ...
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("50205000000")
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("49980000000")

        let delta = formatObj(
            await getValue(vault.functions.get_position_delta(toAddress(user0), toAsset(DAI), toAsset(BTC), false)),
        )
        ...
        delta = formatObj(await getValue(vault.functions.get_position_delta(toAddress(user0), toAsset(DAI), toAsset(BTC), false)))
        expect(delta[0]).eq(false)
        ...
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("50430000000") // 499.8 + 4.5
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("50430000000") // 499.8 + 4.5
        ...
        expect(await getValStr(vaultStorage.functions.get_usdg_amount(toAsset(DAI)))).eq("49980000000") // 499.8
        expect(await getValStr(vaultStorage.functions.get_pool_amounts(toAsset(DAI)))).eq("50230000000") // 502.3

        expect(await getValStr(vaultStorage.functions.get_global_short_sizes(toAsset(BTC)))).eq(toUsd(40))
        expect(await getValStr(vaultStorage.functions.get_global_short_average_prices(toAsset(BTC)))).eq(toNormalizedPrice(40000))
        ...
        await vault
            .connect(user1)
            .functions.increase_position(toAddress(user1), toAsset(DAI), toAsset(BTC), toUsd(200), false)
            .call()

        expect(await getValStr(vaultStorage.functions.get_global_short_sizes(toAsset(BTC)))).eq(toUsd(240))
        ...
        expect(await globalDelta[0]).eq(false)
        expect(await globalDelta[1]).eq(toUsd(2))
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("50430000000") // 502.3 + 2
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("50430000000") // 502.3 + 2
        ...
        delta = formatObj(await getValue(vault.functions.get_position_delta(toAddress(user1), toAsset(DAI), toAsset(BTC), false)))
        ...
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("49853809523") // 502.3 + 1 - 4.76 => 498.53
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("49277619047") // 492.77619047619047619

        await DAI.functions.mint(contrToAccount(vault), expandDecimals(20)).call()
        await vault
            .connect(user2)
            .functions.increase_position(toAddress(user2), toAsset(DAI), toAsset(BTC), toUsd(60), false)
            .call()

        expect(await getValStr(vaultStorage.functions.get_global_short_sizes(toAsset(BTC)))).eq(toUsd(300))
        expect(await getValStr(vaultStorage.functions.get_global_short_average_prices(toAsset(BTC)))).eq(
        ...
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(true))).eq("50003809523") // 500.038095238095238095
        expect(await getValStr(glpManager.functions.get_aum_in_usdg(false))).eq("49277619047") // 492.77619047619047619
        ...
        await vault
            .connect(user2)
            .functions.increase_position(toAddress(user2), toAsset(DAI), toAsset(BNB), toUsd(60), false)
            .call()
    })
})
