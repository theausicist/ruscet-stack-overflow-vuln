...
impl GMX for Contract {
    #[storage(read, write)]
    fn initialize() {
        require(
            !storage.is_initialized.read(), 
            Error::GMXAlreadyInitialized
        );

        storage.is_initialized.write(true);

        storage.name.write_slice(String::from_ascii_str("GMX"));
        storage.symbol.write_slice(String::from_ascii_str("GMX"));
    ...
    */
    ...
    fn transfer_on_behalf_of(
        ...
        require(sender_allowance >= amount, Error::GMXInsufficientAllowance);
...
fn _mint(
    ...
    require(account != ZERO_ACCOUNT, Error::GMXMintToZeroAccount);
...
fn _burn(
    ...
    require(
        msg_amount() == amount,
        Error::GMXInvalidBurnAmountForwarded
    );

    let account_balance = storage.balances.get(account).try_read().unwrap_or(0);
    require(account_balance >= amount, Error::GMXBurnAmountExceedsBalance);
...
fn _transfer(
    ...
    require(sender != ZERO_ACCOUNT, Error::GMXTransferFromZeroAccount);
    require(recipient != ZERO_ACCOUNT, Error::GMXTransferToZeroAccount);

    require(
        amount == msg_amount(),
        Error::GMXInsufficientTransferAmountForwarded
    );

    let sender_balance = storage.balances.get(sender).try_read().unwrap_or(0);
    require(sender_balance >= amount, Error::GMXInsufficientBalance);
...
fn _approve(
    ...
    require(owner != ZERO_ACCOUNT, Error::GMXApproveFromZeroAccount);
    require(spender != ZERO_ACCOUNT, Error::GMXApproveToZeroAccount);
